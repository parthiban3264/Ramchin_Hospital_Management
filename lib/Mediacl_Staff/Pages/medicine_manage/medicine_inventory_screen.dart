...
class _MedicineInventoryScreenState extends State<MedicineInventoryScreen> {
  ...
  List<Medicine> medicines = [
    ...
    Medicine(
      ...
      costPrice: 700.00,
      gstPercent: 12,
      supplierName: 'SpecialMed',
      notes: 'Refrigerate 2-8°C',
    ),
    Medicine(
      id: 5,
  ...
  void _openBulkUpload() {
    showModalBottomSheet(
      ...
      builder: (_) => BulkUploadSheet(
        onImport: (imported) {
          setState(() => medicines.addAll(imported));
          _showSnack(
            '${imported.length} medicine${imported.length == 1 ? '' : 's'} imported!',
            Colors.green.shade700,
          );
        },
      ),
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              setState(() {
                medicines.removeWhere((m) => _selected.contains(m.id));
                _selected.clear();
              });
              Navigator.pop(context);
  ...
  Widget build(BuildContext context) {
    ...
    final outStock = medicines
        .where((m) => m.status == 'Out of Stock' || m.isExpired)
        .length;
            ...
            child: Row(
              children: [
                SizedBox(
                  ...
                  child: Checkbox(
                    value:
                        _selected.length == medicines.length &&
                    ...
                    onChanged: (v) => setState(() {
                      if (v == true)
                        _selected.addAll(medicines.map((m) => m.id));
                      else
                        _selected.clear();
                    }),
                    activeColor: primary,
      ...
      floatingActionButton: Column(
        ...
        children: [
          ...
          FloatingActionButton.small(
            ...
            backgroundColor: Colors.white,
            elevation: 3,
            tooltip: 'Bulk Upload via Excel',
            child: const Icon(Icons.upload_file, color: primary),
          ),
          const SizedBox(height: 10),
          // Add single medicine button
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              fontSize: 11,
              fontWeight: FontWeight.w700,
              color: active ? primary : const Color(0xFF4A5568),
            ),
          ),
          if (active)
            Icon(
  ...
  Widget _buildRow(Medicine m) {
    ...
    Color qtyColor = m.quantity == 0
        ? Colors.red.shade700
        : m.quantity <= m.reorderLevel
        ? Colors.orange.shade700
        : Colors.green.shade700;
    Color expColor = m.isExpired
        ? Colors.red.shade700
        : m.isExpiringSoon
        ? Colors.orange.shade700
        : Colors.black87;
              ...
              Expanded(
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    fontSize: 12,
                    color: expColor,
                    fontWeight: m.isExpired || m.isExpiringSoon
                        ? FontWeight.w600
                        : FontWeight.normal,
                  ),
                ),
  ...
  Widget _statusBadge(String status) {
    ...
    switch (status) {
      ...
      case 'Low Stock':
        ...
        fg = const Color(0xFFA04000);
        break;
      default:
        bg = const Color(0xFFFADBD8);
        fg = const Color(0xFF922B21);
    }
    return Container(
