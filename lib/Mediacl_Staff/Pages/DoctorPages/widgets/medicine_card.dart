...
class _MedicineCardState extends State<MedicineCard>
  ...
  Future<List<dynamic>> getDispense(int id) async {

    return ConsultationService.getDispense(id);
  ...
  void initState() {
    ...
    if (widget.initialSavedMedicines.isNotEmpty) {
      ...
      for (var saved in widget.initialSavedMedicines) {
        ...
        entry.daysController.text = (saved['days'] ?? 0).toString();
  ...
  void _addNewEntry() {
    ...
    newEntry.currentMedicine['morning'] = false;

    newEntry.currentMedicine['afternoon'] = false;

    newEntry.currentMedicine['night'] = false;
  ...
  void _fetchSuggestions(String query, int index) {
    ...
    // Build a set of medicineIds that are already in the current card data,

    // so we don't suggest the same medicine name again for new rows.

    final existingMeds = _getNonNullMedicines();

    final usedIds = existingMeds
    ...
    final filtered = widget.allMedicines.where((m) {
      ...
      // Exclude already-used medicines from suggestions
  ...
  Future<void> _selectSuggestion(Map<String, dynamic> med, int index) async {

    final entry = medicineEntries[index];

    final medicineId = med['id'];
    ...
    try {
      ...
      final List<dynamic> dispenseData = await getDispense(medicineId);
    ...
    setState(() {
      ...
      entry.currentMedicine['medicineId'] = medicineId;
      ...
      final batches = med['batches'] ?? [];

      //I/flutter (21462): batches [{id: 33, hospital_Id: 1, medicine_id: 1, HSN: G12, batch_no: 1, expiry_date: 2026-05-12T18:30:00.000Z, manufacture_date: 2026-02-02T18:30:00.000Z, total_stock: 0, total_quantity: 105, quantity: 100, free_quantity: 5, unit: 1, rack_no: 10, mrp: 10, profit: 50, purchase_price_unit: 5.05, purchase_price_quantity: 5.05, selling_price_quantity: 7.57, selling_price_unit: 7.57, purchase_details: {base_amount: 500, gst_percent: 1, purchase_date: 2026-02-03T14:34:41.834183, purchase_price: 505, gst_per_quantity: 0.05, total_gst_amount: 5, rate_per_quantity: 5}, supplier_id: 1, is_active: true, created_at: 2026-02-03T09:07:51.618Z}, {id: 34, hospital_Id: 1, medicine_id: 1, HSN: G13, batch_no: 2, expiry_date: 2026-05-04T18:30:00.000Z, manufacture_date: 2026-02-02T18:30:00.000Z, total_stock: 2431, total_quantity: 500, quantity: 500, free_quantity: 0, unit: 5, rack_no: 10e, mrp: 15, profit: 20, purchase_price_unit: 2.2, purchase_price_quantity: 11, selling_price_quantity: 13.2, selling_price_unit: 2.63, purchase

      final int totalBatchStock = (batches as List).fold(

        0,

        (sum, b) =>

            sum + (int.tryParse(b['total_stock']?.toString() ?? '0') ?? 0),

      );



      // Available stock = Total - Dispensed

      final int availableStock = (totalBatchStock - totalDispensed).clamp(
      ...
      entry.currentMedicine['stock'] = availableStock;
  ...
  //   final isTablets = (med['route'] as String?)?.toLowerCase() == 'tablets';

  //   // final isTabletsAndSyrups =

  //   //     isTablets && (med['route'] as String?)?.toLowerCase() == 'syrups';

  //   final route = (med['route'] ?? '').toString().toLowerCase();

  //   final isTabletOrSyrup = route == 'tablets' || route == 'syrups';
  ...
  //   // STOCK VALIDATION: Prevent prescribing more than available

  //   if (med['base_total_stock'] != null) {

  //     final int availableStock = med['base_total_stock'] as int;

  //     if (tabletsToCharge > availableStock) {

  //       return null; // Return null if over-stock, so it's not "saved" or added to prescription
  ...
  //     "total": double.parse(totalCost.toStringAsFixed(2)),
  ...
  Map<String, dynamic>? _buildIfValid(_MedicineEntry entry) {
    ...
    //print('mediii entry $entry');

    final med = entry.currentMedicine;

    print('mediii $med');

    // print('mediiiBatch ${med['batches']}');
    ...
    // ❌ Basic required fields for ALL medicines
    ...
    // ❌ Tablets & Syrups → AC/PC + Dose timing mandatory

    if (isTabletOrSyrup) {

      if (afterEat == null || !hasDose || days <= 0) {
    ...
    final dosesPerDay = isTabletOrSyrup ? _dosesPerDay(med) : 1;
    ...
    if (med['stock'] != null) {
      ...
      final int availableStock = med['stock'] as int;
    ...
    return {
      ...
      "total_quantity": tabletsToCharge,
    ...
    return SizedBox(
      ...
      child: TextField(
        ...
        onChanged: (val) {
          ...
          int inputDays = int.tryParse(val) ?? 0;

          int clampedDays = inputDays;



          // Clamp days based on original stock and dosage if a medicine is selected.

          if (key == 'days' && entry.currentMedicine['stock'] != null) {

            final int totalStock = entry.currentMedicine['stock'] as int;



            final qtyPerDose = _parseQtyText(entry.quantityController.text);

            final dosesPerDay = _dosesPerDay(entry.currentMedicine);

            final dailyUsage = (qtyPerDose * dosesPerDay).round();



            if (dailyUsage > 0 && totalStock > 0) {

              final maxDays = totalStock ~/ dailyUsage;

              if (clampedDays > maxDays) {
          ...
          // Update text to clamped value, so the user sees the max.

          if (clampedDays != inputDays) {

            entry.daysController.text = clampedDays.toString();

            entry.daysController.selection = TextSelection.fromPosition(

              TextPosition(offset: entry.daysController.text.length),
          ...
          setState(() {
            ...
            entry.currentMedicine[key] = clampedDays;
                  ...
                  child: CompositedTransformTarget(
                    ...
                    child: TextField(
                      ...
                      onChanged: (v) {
                        ...
                        entry.daysController.text = '';

                        entry.currentMedicine['days'] = 0;

                        entry.currentMedicine['weeks'] = 0;

                        entry.currentMedicine['months'] = 0;
