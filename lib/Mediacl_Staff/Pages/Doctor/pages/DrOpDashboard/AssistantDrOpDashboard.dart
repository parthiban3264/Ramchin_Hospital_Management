...
class AssistantDrOpDashboardPage extends StatefulWidget {
  ...
  @override
  State<AssistantDrOpDashboardPage> createState() =>
      _AssistantDrOpDashboardPageState();
}
...
class _AssistantDrOpDashboardPageState
  ...
  //     final assignDoctorId = staff['AssignDoctorId']?.toString();
  //
  ...
  //       break; // optional if only one match is needed
  //     }
  //   }
  //
  //   final profile = await AdminService().getProfileAssignDr(assignDoctorId);
  ...
  Future<void> _loadDrOpData() async {
    ...
    for (var staff in doctorData) {
      final staffUserId = staff['user_Id']?.toString().trim();
