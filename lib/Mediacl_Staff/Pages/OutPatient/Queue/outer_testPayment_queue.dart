...
class FeesTestQueuePage extends StatefulWidget {
  ...
  @override
  State<FeesTestQueuePage> createState() => _FeesTestQueuePageState();
}

class _FeesTestQueuePageState extends State<FeesTestQueuePage> {
  ...
  void _loadFees() {
    _feesFuture = PaymentService().getAllPendingNewTestFees().then((list) {
      // Normalize status
  ...
  // void _loadFees() {
  //   _feesFuture = PaymentService().getAllPendingNewTestFees().then((list) {
  //     // ✅ Normalize + filter Test-only consultations
  //     final List<dynamic> normalized = list
  //         .where(
  //           (e) =>
  //               e['Consultation'] != null &&
  //               e['Consultation']['isTestOnly'] == true,
  //         )
  //         .map((e) {
  //           e["status"] = e["status"].toString().toLowerCase();
  //           return e;
