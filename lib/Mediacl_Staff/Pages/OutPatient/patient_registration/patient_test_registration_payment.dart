...
class TestRegistrationAndPayment extends StatefulWidget {
  const TestRegistrationAndPayment({super.key});

  @override
  State<TestRegistrationAndPayment> createState() =>
      TestRegistrationAndPaymentState();
}

class TestRegistrationAndPaymentState
    extends State<TestRegistrationAndPayment> {
  ...
  void _submitPatientData() async {
    try {
      ...
      // setState(() {
      //   //paymentData = payment;
      //   paymentPatient = patientResult['Patient'] ?? {};
  ...
  Future<void> _submitTestsOrScans({
    ...
    required bool isScan, // true = scan, false = test
  }) async {
    ...
    if (items.isEmpty) {
      _showSnacks("No ${isScan ? 'scans' : 'tests'} selected!", isError: true);
      return;
    }
    ...
    try {
      ...
      for (var entry in items.entries) {
        ...
        final Map<String, dynamic> amounts = Map<String, dynamic>.from(
          data['amounts'] ?? data['selectedOptionsAmount'] ?? {},
        );
        ...
        // ✅ capture payment only once

        final test = decoded?['test'];
        if (test != null) {
          _collectedTestScanItems.add(test);
        }
        // _sharedPayment ??= decoded['data']?['payment'];
        final latestPayment = decoded['data']?['payment'];
        ...
        setState(() {
          paymentData = {
            ...
            "TestingAndScanningPatients":
                _collectedTestScanItems, // ✅ attach here
          };
          paymentPatient = _sharedPayment?['Patient'] ?? {};
          showPaymentPage = true;
        });
      }
    } catch (e) {
      _showSnacks(
        'Error submitting ${isScan ? 'scans' : 'tests'}',
        isError: true,
      );
  ...
  Widget build(BuildContext context) {
    ...
    return _buildTestRegistrationPage(); // your existing UI
  }
  ...
  Widget _buildTestRegistrationPage() {
