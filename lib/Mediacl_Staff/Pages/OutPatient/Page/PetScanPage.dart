...
class PetScanPage extends StatefulWidget {
  final Map<String, dynamic> record;
  final int mode;
  // final Function(bool) onRefresh;

  const PetScanPage({super.key, required this.record, required this.mode});

  @override
  State<PetScanPage> createState() => _PetScanPageState();
}

class _PetScanPageState extends State<PetScanPage>
  ...
  void _handleSubmit() async {
    ...
    if (hasEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Please fill all PET-Scan values."),
          backgroundColor: Colors.redAccent,
        ),
      );
      return;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('PET-Scan marked as Completed ✅'),
  ...
  void handelSubmitReport() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('PET-Scan Report Submitted ✅'),
            backgroundColor: primaryColor,
          ),
        );
        Navigator.pop(context, true);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('$e Error: failed to submit PET-Scan Report'),
            backgroundColor: primaryColor,
          ),
        );
      }
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: "Enter PET-Scan report or notes...",
  ...
  Widget _buildMedicalCard({
    ...
    // Collect selected PET-Scan options
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(height: 30, color: Colors.grey),
          _sectionHeader("Selected PET-Scan Options"),
          const SizedBox(height: 10),

          if (selectedOptions.isEmpty)
            const Text(
              "No PET-Scan Options Selected",
