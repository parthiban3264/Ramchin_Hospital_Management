...
class MriScanPage extends StatefulWidget {
  ...
  const MriScanPage({super.key, required this.record, required this.mode});
...
class _MriScanPageState extends State<MriScanPage>
  ...
  void _handleSubmit() async {
    ...
    if (hasEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Please fill all MriScan values."),
          backgroundColor: Colors.redAccent,
        ),
      );
      return;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('MriScan marked as Completed ✅'),
  ...
  void handelSubmitReport() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('MriScan Report Submitted ✅'),
            backgroundColor: primaryColor,
          ),
        );
        Navigator.pop(context, true);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('$e Error: failed to submit MriScan Report'),
            backgroundColor: primaryColor,
          ),
        );
      }
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: "Enter MriScan report or notes...",
  ...
  Widget _buildMedicalCard({
    ...
    // Collect selected MriScan options
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(height: 30, color: Colors.grey),
          _sectionHeader("Selected MriScan Options"),
          const SizedBox(height: 10),

          if (selectedOptions.isEmpty)
            const Text(
              "No MriScan Options Selected",
              ...
              Center(
                child: ElevatedButton.icon(
                  onPressed: () async {
                    ...
                    if (picked != null) {
                      if (pickedImages.length + 1 > 6 && mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text("Maximum 6 images allowed!"),
                            backgroundColor: Colors.redAccent,
