...
import '../Queue/ECHOQueuePage.dart';
...
class EchoPage extends StatefulWidget {
  ...
  const EchoPage({Key? key, required this.record, required this.mode})
    : super(key: key);
...
class _EchoPageState extends State<EchoPage>
  ...
  void _handleSubmit() async {
    ...
    if (hasEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Please fill all ECHO values."),
          backgroundColor: Colors.redAccent,
        ),
      );
      return;
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('ECHO marked as Completed ✅'),
  ...
  void handelSubmitReport() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('ECHO Report Submitted ✅'),
          backgroundColor: primaryColor,
        ),
      );
      Navigator.pop(context, true);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('$e Error: failed to submit ECHO Report'),
          backgroundColor: primaryColor,
        ),
      );
    } finally {
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: "Enter ECHO report or notes...",
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(height: 30, color: Colors.grey),
          _sectionHeader("Selected ECHO Options"),
          const SizedBox(height: 10),

          if (selectedOptions.isEmpty)
            const Text(
              "No ECHO Options Selected",
