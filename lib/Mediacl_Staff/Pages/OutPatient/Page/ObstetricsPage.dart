...
class ObstetricsPage extends StatefulWidget {
  ...
  const ObstetricsPage({super.key, required this.record, required this.mode});
...
class _ObstetricsPageState extends State<ObstetricsPage>
  ...
  void _handleSubmit() async {
    ...
    if (hasEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Please fill all OBSTETRICS values."),
          backgroundColor: Colors.redAccent,
        ),
      );
      return;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('OBSTETRICS marked as Completed ✅'),
  ...
  void handelSubmitReport() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('OBSTETRICS Report Submitted ✅'),
            backgroundColor: primaryColor,
          ),
        );
        Navigator.pop(context, true);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('$e Error: failed to submit OBSTETRICS Report'),
            backgroundColor: primaryColor,
          ),
        );
      }
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: "Enter OBSTETRICS report or notes...",
  ...
  Widget _buildMedicalCard({
    ...
    // Collect selected OBSTETRICS options
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(height: 30, color: Colors.grey),
          _sectionHeader("Selected OBSTETRICS Options"),
          const SizedBox(height: 10),

          if (selectedOptions.isEmpty)
            const Text(
              "No OBSTETRICS Options Selected",
