...
class XRayPage extends StatefulWidget {
  ...
  const XRayPage({super.key, required this.record, required this.mode});
...
class _XRayPageState extends State<XRayPage>
  ...
  void _handleSubmit() async {
    ...
    if (hasEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Please fill all X-Ray values."),
          backgroundColor: Colors.redAccent,
        ),
      );
      return;
  ...
  void handelSubmitReport() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('X-Ray Report Submitted ✅'),
            backgroundColor: primaryColor,
          ),
        );
        Navigator.pop(context, true);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('$e Error: failed to submit X-Ray Report'),
            backgroundColor: primaryColor,
          ),
        );
      }
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: "Enter X-Ray report or notes...",
