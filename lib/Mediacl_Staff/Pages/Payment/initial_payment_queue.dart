...
import 'package:hospitrax/Mediacl_Staff/Pages/Payment/widget/initial_payment_create_page.dart';
import 'package:intl/intl.dart';
import 'package:lottie/lottie.dart';
import '../../../Pages/NotificationsPage.dart';
import '../../../Services/payment_service.dart';
import 'PaymentPage.dart';
import 'ct-scan_payment_page.dart';
import 'initial_payment_page.dart';

class InitialFeesQueuePage extends StatefulWidget {
  const InitialFeesQueuePage({super.key});

  @override
  State<InitialFeesQueuePage> createState() => _InitialFeesQueuePageState();
}

class _InitialFeesQueuePageState extends State<InitialFeesQueuePage> {
  ...
  void _loadFees() {
    _feesFuture = PaymentService().getAllOnlyInitialPendingFees().then((list) {
      // Normalize status
  ...
  void _applyHistoryFilter() {
    ...
    // 🔍 IF SEARCH IS ACTIVE → SHOW ALL MATCHES (IGNORE DATE)
    if (search.isNotEmpty) {
      ...
      filtered = filtered.where((e) {
        ...
        if (historyFilter == 'Today') {
          ...
          return isLastTwoDays(date); // 👈 ONLY LAST 2 DAYS
        }
      }).toList();
    }
  ...
  void _applyQueueFilter() {
    ...
    _queueFees = _allFees.where((e) {
      ...
      try {
        final date = DateFormat('yyyy-MM-dd hh:mm a').parse(e['createdAt']);
        ...
        if (queueFilter == 'Today') {
          ...
          // ✅ ONLY LAST 3 DAYS (excluding today)
          return date.isBefore(todayStart) && date.isAfter(threeDaysAgo);
        }
      } catch (_) {
        return false;
      }
    ...
    // ⬆️ Oldest first (queue order)
    _queueFees.sort((a, b) {
      try {
        return DateFormat('yyyy-MM-dd hh:mm a')
            .parse(a['createdAt'])
            .compareTo(DateFormat('yyyy-MM-dd hh:mm a').parse(b['createdAt']));
      } catch (_) {
        return 0;
      }
    });
  ...
  DateTime? parseAnyDate(dynamic value) {
    ...
    // Primary format: 2026-01-10 03:56 PM
    try {
  ...
  bool isLastTwoDays(DateTime date) {
    final todayStart = DateTime(
      DateTime.now().year,
      DateTime.now().month,
      DateTime.now().day,
    );
    final twoDaysAgo = todayStart.subtract(const Duration(days: 1));

    return date.isBefore(todayStart) && date.isAfter(twoDaysAgo);
  }
                        ...
                        onTap: _currentIndex == 2
                            ...
                            : () async {
                                ...
                                final result = await Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (_) => InitialFeesPaymentPage(
                                      fee: item,
                                      patient: patient,
                                      index: _currentIndex,
                                      page: 'queue',
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => InitialPaymentCreatePage()),
          );
        },
